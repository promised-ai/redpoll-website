...
RV &mdash; which stands for **R**andom **V**ariables &mdash; is a crate for random variables in rust. It's designed for developing probabilistic programming tools.
But luckily we want it to do a lot and be easy to use.
...
We have built in a few examples of conjugate priors to enable simple conjugate analysis. For example, if we wanted to infer the unknown weight of an coin given a set of observations we could model the flipping of the coin as a Bernoulli distribution and our prior belief about the weights of coins as a Beta. Beta is conjugate to Bernoulli, which means that the posterior distribution is also Beta. So we can make posterior inferences about the coin weight without fussing with a Bernoulli distribution.

In `rv`, we can implement a `ConjugatePrior` trait for `Beta`
...
In this case, we implements `ConjugatePrior<bool, Bernoulli>` for `Beta`. Yes, `Beta` implements `Rv<Bernoulli>`.
...
# Roadmap
 ...
 The development philosophy of `rv` stems from a Björk quote that has influenced how I think when I create things:
...
Selfishness. It should do what we want it to do. 
